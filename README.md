...
- **Shared translations**: before calling the model, a request reuses the newest translation of the same source text into the same target language made by any other user (streaming and non-streaming). The response is tagged `fromSharedTranslation`, and a personal cache hit is tagged `fromCache`; the web UI shows a matching badge. A `skipCache` request bypasses both and forces a fresh model call — the web UI's "Re-translate" button.
...
The `tranShilator-plugin/` directory contains the **Pixiv Novel Translator** Chrome/Edge Manifest V3 extension. The current manifest version is `1.4.3` (build metadata in `version.js` is generated from the release commit).
...
- Shared translations: served from `translation_records` by exact source-text and resolved target-language match across users; legacy rows without a `targetLanguage` never match, so nothing is served in the wrong language.
