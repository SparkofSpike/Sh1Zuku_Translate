  ...
  <div class="ocr-preview">
    <div class="ocr-preview-grid">
      <div v-for="(item, index) in previews" :key="index" class="ocr-preview-item">
        <img :src="item" :alt="t('components.ocrPreview.imageAlt', { index: index + 1 })" />
        <button
          class="ocr-preview-remove"
          type="button"
          :title="t('components.ocrPreview.removeImage')"
    ...
    <template v-if="mode === 'ocr'">
      <div class="ocr-preview-actions">
        <button class="btn-sm btn-remove" @click="$emit('clear')">
          {{ previews.length > 1 ? t('components.ocrPreview.removeAll') : t('components.ocrPreview.remove') }}
        </button>
        <button class="btn-sm btn-primary" @click="$emit('ocr')" :disabled="loading">
          {{ loading ? t('components.ocrPreview.recognizing') : (previews.length > 1 ? t('components.ocrPreview.paddleOcrWithCount', { count: previews.length }) : t('components.ocrPreview.paddleOcr')) }}
        </button>
      </div>
      <label style="margin-top:8px;display:inline-flex;align-items:center;gap:6px;font-size:13px;cursor:pointer;">
        ...
        {{ t('components.ocrPreview.repairSegments') }}
      </label>
      <div style="margin-top:8px;display:flex;align-items:center;gap:8px;font-size:13px;">
        <span>{{ t('components.ocrPreview.thresholdLabel') }}</span>
    ...
    <div v-else-if="previews.length > 1" class="ocr-preview-actions">
      <button class="btn-sm btn-remove" @click="$emit('clear')">{{ t('components.ocrPreview.removeAll') }}</button>
    </div>
  </div>
</template>
